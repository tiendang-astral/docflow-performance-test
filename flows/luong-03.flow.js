/**
 * Luồng 03 — Tạo quy tắc
 *
 * Steps:
 *   01  POST /api/v1/auth/login           — đăng nhập
 *   02  GET  /api/v1/rules                — xem danh sách quy tắc
 *   03  POST /api/v1/rules                — tạo quy tắc mới
 *   04  GET  /api/v1/rules/{id}           — xem chi tiết quy tắc vừa tạo
 *   05  PUT  /api/v1/rules/{id}           — cập nhật quy tắc
 *   06  DELETE /api/v1/rules/{id}         — xoá quy tắc (tự dọn dẹp)
 */
...
export default function runFlow(users) {
  ...
  group('03-create-rule', () => {
    const payload = {
      name: `LT-Rule-VU${__VU}-I${__ITER}-${Date.now()}`,
  ...
  group('05-update-rule', () => {
    const payload = {
      name: `LT-Rule-VU${__VU}-I${__ITER}-Updated`,
