/**
 * Stress test — PUT /v1/rules/{id}
 *
 * Strategy: setup tạo N "victim" rule (1 cái / VU), mỗi VU chỉ update VICTIM
 * CỦA RIÊNG MÌNH → tránh row contention.
 *
 * Chạy:
 *   k6 run tests/rules/stress/update.js
 *   k6 run -e MAX_VU=10 tests/rules/stress/update.js
 */
...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{name:rules_update}': ['p(95)<2500'],
  },
};
...
export function setup() {
  ...
  const runId = `_stress_update_rule_${Date.now()}`;
  console.log(`setup: tạo ${VICTIM_COUNT} victim rules (${runId})...`);
...
export default function ({ tokens, victimIds }) {
  ...
  const res = http.put(`${RULES_URL}/${id}`, payload,
...
export function teardown({ tokens, victimIds }) {
  console.log(`teardown: xóa ${victimIds.length} victim rules...`);
  let deleted = 0;
  let failed = 0;
...
export const handleSummary = buildSummary('rules-update-stress');
